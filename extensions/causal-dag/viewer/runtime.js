// Extracted from the design-component runtime and hardened for offline export.
"use strict";
(() => {
  ...
  function collectProps(node, kind, host) {
    ...
    for (const { name, value } of [...node.attributes]) {
      ...
      if (key.startsWith("data-dc-bind-"))
        key = kebabToCamel(key.slice("data-dc-bind-".length));
  ...
  function createExternalModules(onResolved) {
    ...
    function load(kind, url, after) {
      ...
      const p = Promise.resolve(after).then(() => {
        const e = new Error("external modules are disabled in self-contained causal-DAG exports");
        cache.set(url, {
          mod: {},
          globals: {},
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      r.fetched = true;
      console.error(
        "[dc-runtime] sibling components are disabled in self-contained causal-DAG exports:",
        name
      );
    }
    let rootName = null;
  ...
  function loadReactUmd() {
    ...
    return Promise.reject(new Error("self-contained viewer is missing bundled React assets"));
  }
  function init() {
