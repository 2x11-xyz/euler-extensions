...
class Host:
    ...
    def spawn_agents(self, tasks: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
        result = self._request(
            "euler/host/spawn-agents", {"tasks": [dict(task) for task in tasks]}
        )
...
def serve(handlers: Mapping[str, CommandHandler]) -> None:
    ...
    if not isinstance(command_params, dict) or not isinstance(
        command_params.get("command"), str
    ):
